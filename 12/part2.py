...
def find_unique_paths(G, v, path=None, visited_small_caves=None, visited_small_cave_twice=False):
    if path is None:
        path = []
        ...
        if is_small_cave(v):
            if not visited_small_cave_twice:
                for u in filter(lambda x: x != START and x not in visited_small_caves, G[v]):
                    unique_paths |= find_unique_paths(G, u, path, visited_small_caves, True)
            visited_small_caves.append(v)
        for u in filter(lambda x: x != START and x not in visited_small_caves, G[v]):
            unique_paths |= find_unique_paths(G, u, path, visited_small_caves, visited_small_cave_twice)
